...
class FeatureWiseAffine(nn.Module):
    def __init__(self, in_channels, out_channels, use_affine_level=False):
        super(FeatureWiseAffine, self).__init__() # same as super().__init__()
...
# class ardasample(nn.Module):
#     def __init__(self, dim) -> None:
#         super().__init__()
#         self.arda = nn.Conv2d(dim, dim, 3, 2)
#     def forward(self, x):
#         return self.arda(x)
    ...
    def train(self, epoch, verbose):
        ...
        for i in tqdm(range(epoch)):
            ...
            if (i+1) % verbose == 0:
                ...
                self.sr3.train()
                print("time for inference " + str(time.time() - tic))
                tic = time.time()
                train_loss = train_loss / len(self.dataloader)
                print(f'Epoch: {i+1} / loss:{train_loss:.3f} / val_loss:{val_loss.item():.3f}')
                print("time for val loss calc " + str(int(time.time() - tic)))
                tic = time.time()
                ...
                plt.savefig(f"SuperResolution_Result{i+1}.jpg")
                plt.close()
                print("time for matplotlib " + str(time.time() - tic))
                # Save model weight
...
if __name__ == "__main__":
    ...
    testloader = DataLoader(torchvision.datasets.ImageFolder(root, transform=transforms_), 
